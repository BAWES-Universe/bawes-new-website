...
const features = [
  { key: 'proximityChat', icon: 'spatial_audio' },
  { key: 'textChat', icon: 'chat' },
  { key: 'matrixChat', icon: 'hub' },
  { key: 'megaphone', icon: 'campaign' },
  { key: 'meetingRooms', icon: 'video_chat' },
  { key: 'screenSharing', icon: 'present_to_all' },
  { key: 'broadcastingEvents', icon: 'podcasts' },
  { key: 'playAudio', icon: 'music_note' },
  { key: 'follow', icon: 'group_work' },
  { key: 'lockModerate', icon: 'verified_user' },
]
    ...
    <div className="pt-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-0 pb-2">
        <div className="flex items-center gap-2 text-xs text-text-muted">
          <Link href="/features-overview" className="hover:text-primary transition-colors">{ft('breadcrumbFeatures')}</Link>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {features.map((feature, i) => {
              ...
              const descKey = feature.key + 'Desc'
              return (
              <Link key={feature.key} href={featureSlugs[t(titleKey)] || `/features/${feature.key}`} className="group">
              <motion.div
